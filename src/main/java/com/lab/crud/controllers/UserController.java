...
public class UserController {
	...
	@GetMapping("/edits")
	public String edits(Model model, @RequestParam("id") long id) {
		model.addAttribute("user", userdao.findById(id).get());
		model.addAttribute("view", "/WEB-INF/user/updateinfo.jsp");
		return "layout";
	}

	@PostMapping("/updates")
	public String updates(Model model, @RequestParam("idUserUpdate") long id, Users user) {
		user.setId_user(id);
		userdao.save(user);
	...
	@GetMapping("/deletes")
	public String deleteGets(Model model, @RequestParam("id") long id) {
		userdao.delete(userdao.findById(id).get());
		return "redirect:/user/inforuser";
	}

	@GetMapping("/listusers")
	public String listUsers(Model model, @RequestParam("userId") Long userId) {
	    Optional<Users> userOptional = userdao.findById(userId); // Tìm người dùng theo ID
	    if (userOptional.isPresent()) {
	        Users user = userOptional.get();
	        model.addAttribute("listUser", user);
	        model.addAttribute("view", "/WEB-INF/user/inforuser.jsp");
	    } 
	    return "layout";
	}
